...
def save_cv_metrics(fold_histories, plots_dir):
    ...
    cv_metrics = {
        "cross_validation": {
            ...
            "avg_best_val_loss": sum(h["best_val_loss"] for h in fold_histories)
            / len(fold_histories),
            "avg_best_val_accuracy": sum(h["best_val_accuracy"] for h in fold_histories)
            / len(fold_histories),
            "fold_details": [],
        }
    }
...
def test_with_mlflow(
    ...
    run_name = (
        f"test_{backbone}_freeze_{freeze_backbone}_{datetime.now().strftime('%H%M%S')}"
    )
...
def main(args):
    ...
    if args.mode == "train":
        ...
        for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset)):
            ...
            val_loader = DataLoader(
                dataset, batch_size=config.BATCH_SIZE, sampler=val_subsampler
            )
            ...
            optimizer = torch.optim.Adam(
                model.parameters(), lr=config.params["train"]["learning_rate"]
            )

            # Log fold information
            logging.info(
                f"""Fold {fold + 1} Details:
    ...
    MLflow:          {'ENABLED' if args.use_mlflow else 'DISABLED'}
        ...
        if fold_histories:
            avg_best_val_loss = sum(h["best_val_loss"] for h in fold_histories) / len(
                fold_histories
            )
            avg_best_val_accuracy = sum(
                h["best_val_accuracy"] for h in fold_histories
            ) / len(fold_histories)
        ...
        if test_results:
            ...
            if hasattr(args, "min_accuracy") and args.min_accuracy:
                ...
                if accuracy >= args.min_accuracy:
                    logging.info(
                        f"✓ Accuracy ({accuracy:.2%}) meets threshold ({args.min_accuracy:.2%})"
                    )
                else:
                    logging.error(
                        f"✗ Accuracy ({accuracy:.2%}) below threshold ({args.min_accuracy:.2%})"
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PyTorch Classification with DVC & MLflow"
    )
    parser.add_argument(
        "--mode",
        type=str,
